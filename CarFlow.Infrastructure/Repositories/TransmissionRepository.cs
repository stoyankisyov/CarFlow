...
using TransmissionVariant = CarFlow.Core.Models.TransmissionVariant;
...
public class TransmissionRepository(CarFlowContext context) : ITransmissionRepository
{
    ...
    public async Task<TransmissionVariant?> GetVariantAsync(int id)
    {
        var variant = await context.TransmissionVariants.SingleOrDefaultAsync(x => x.Id == id);

        return variant?.ToDomainModel();
    }

    public async Task<List<TransmissionVariant>> GetAllVariantsAsync()
    {
    ...
    public async Task UpdateAsync(Core.Models.Transmission updateTransmission)
    {
        ...
        {
            var updateTransmissionVariant =
                updateTransmission.TransmissionVariants.FirstOrDefault(x => x.Id == existingTransmissionVariant.Id);
        ...
        var newTransmissionVariants = updateTransmission.TransmissionVariants
            .Where(updateTransmissionVariant => existingTransmission.TransmissionVariants.All(
                existingTransmissionVariant => existingTransmissionVariant.Id != updateTransmissionVariant.Id))
            .ToList();
