...
public class RegionService(IRegionRepository regionRepository) : IRegionService
{
    ...
    public async Task<Region?> GetAsync(int id)
        => await regionRepository.GetAsync(id);

    public async Task<Page<Region>> GetPageAsync(int currentPage, int pageSize)
        => await regionRepository.GetPageAsync(currentPage, pageSize);

    public async Task UpdateAsync(Region updateRegion)
        => await regionRepository.UpdateAsync(updateRegion);
}